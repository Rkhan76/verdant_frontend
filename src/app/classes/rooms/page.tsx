...
const roomsData = [
  { sl: '01', roomNo: '101', capacity: '40', status: 'Active' },
  { sl: '02', roomNo: '102', capacity: '45', status: 'Inactive' },
  { sl: '03', roomNo: '103', capacity: '35', status: 'Active' },
  { sl: '04', roomNo: '104', capacity: '50', status: 'Inactive' },
  { sl: '05', roomNo: '105', capacity: '40', status: 'Active' },
  { sl: '06', roomNo: '106', capacity: '45', status: 'Inactive' },
  { sl: '07', roomNo: '107', capacity: '35', status: 'Active' },
  { sl: '08', roomNo: '108', capacity: '50', status: 'Inactive' },
  { sl: '09', roomNo: '109', capacity: '40', status: 'Active' },
  { sl: '10', roomNo: '110', capacity: '45', status: 'Inactive' },
];
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h1 className="text-[22px] font-semibold text-gray-800 tracking-tight">Class Room</h1>
          <p className="text-sm text-gray-500 mt-1 flex items-center gap-2">
            <Link href="/" className="hover:text-[#25a194] transition-colors">Dashboard</Link>
            <span>/</span>
            <Link href="/classes" className="hover:text-[#25a194] transition-colors">Classes</Link>
            <span>/</span>
            <span className="text-gray-400">Class Room</span>
          </p>
        </div>
          ...
          <table className="w-full text-sm text-left whitespace-nowrap">
            <thead className="text-xs text-gray-800 bg-gray-50/50 uppercase font-semibold border-b border-gray-100">
              <tr>
                ...
                <th scope="col" className="px-4 py-4 min-w-[300px]">Room No</th>
                <th scope="col" className="px-4 py-4 min-w-[200px]">Capacity</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {roomsData.map((row, i) => (
                <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-4 py-4 text-gray-500">{row.roomNo}</td>
                  <td className="px-4 py-4 text-gray-500">{row.capacity}</td>
                  <td className="px-4 py-4">
                    <span className={`px-4 py-1.5 rounded text-xs font-semibold ${row.status === 'Active' ? 'bg-[#10b981]/10 text-[#10b981]' : 'bg-red-50 text-red-500'}`}>
                      {row.status}
                    </span>
                  </td>
                  <td className="px-4 py-4">
