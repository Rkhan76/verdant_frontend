...
import { Search, Moon, Bell, ChevronRight, User, HelpCircle, Settings, LogOut } from 'lucide-react';
  ...
  return (
    <header className="sticky top-0 z-30 flex h-[72px] items-center justify-between bg-white px-8 border-b border-gray-100 transition-all">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="h-8 border-l border-gray-200 mx-2 hidden sm:block"></div>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger className="outline-none focus:outline-none focus-visible:ring-2 focus-visible:ring-[#25a194] focus-visible:ring-offset-2 rounded-lg flex items-center">
            <div className="bg-gray-50 rounded-lg p-2 pr-3 flex items-center gap-3 border border-gray-100 cursor-pointer hover:bg-gray-100 transition-colors">
              <Avatar className="h-9 w-9">
                <AvatarImage src="https://i.pravatar.cc/150?u=admin" />
                <AvatarFallback>JC</AvatarFallback>
              </Avatar>
              <div className="hidden md:block text-left">
                <p className="text-sm font-semibold text-gray-800 leading-tight">Jone Copper</p>
                <p className="text-xs text-gray-500">Admin</p>
              </div>
              <ChevronRight className="h-4 w-4 text-gray-400 shrink-0 hidden md:block" />
            </div>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-56 mt-1 p-2 bg-white rounded-xl shadow-lg border-gray-100">
            ...
            <DropdownMenuItem className="flex items-center gap-3 cursor-pointer p-2.5 rounded-lg text-red-500 hover:bg-red-50 hover:text-red-600 focus:bg-red-50 focus:text-red-600">
